...
import { fetchVerifyOTPThunk, fetchSendOTPThunk } from '../redux/features/user/userThunk';
...
const handleSubmit = async (e) => {
  ...
  const result = await dispatch(fetchVerifyOTPThunk({ email, otp: otpCode }));
  ...
  const handleResendOTP = async () => {
    setIsLoading(true);
    try {
      const result = await dispatch(fetchSendOTPThunk({ email }));
      if (result.error) {
        toast.error(result.payload);
      } else {
      ...
      <div className="min-h-screen flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8 relative overflow-hidden">
        ...
        <div className="max-w-md w-full space-y-8 relative z-10">
          <div className="text-center animate-fade-in">
            <h2 className="text-4xl font-bold gradient-text mb-2">Verify OTP ✨</h2>
          ...
          <div className="glass-card p-8 animate-slide-up glow-effect">
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="flex justify-center gap-4">
                {otp.map((value, index) => (
                  <input
                    key={index}
                    id={`otp-${index}`} // Thêm id để xác định ô nhập
                    type="text"
                    maxLength="1"
            ...
            <div className="text-center mt-4">
              {timer > 0 ? (
                <p className="text-white/70 text-sm">Resend OTP in <span className="font-bold">{Math.floor(timer / 60)}:{String(timer % 60).padStart(2, '0')}</span></p>
              ) : (
                <button
